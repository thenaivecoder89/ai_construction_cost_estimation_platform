...
def build_embedding_input_texts(batch_df, max_input_chars):
    input_texts = []

    for _, row in batch_df.iterrows():
        chunk_text = clean_text(row["chunk_text"])

        # Safety cap to keep each OpenAI embedding request under the model's
        # aggregate token/request-size limit when a batch has long chunks.
        chunk_text = chunk_text[:int(max_input_chars)]
...
def iter_embed_chunks(rebuild_inventory: str = "Y"):
    ...
    batch_size = positive_int_env("EMBED_BATCH_SIZE", 50)
    ...
    max_embedding_input_chars = positive_int_env("EMBED_MAX_INPUT_CHARS_PER_CHUNK", 6000)
    min_embedding_input_chars = positive_int_env("EMBED_MIN_INPUT_CHARS_PER_CHUNK", 1000)
    ...
    adaptive_request_splits = 0
    adaptive_input_truncations = 0
    ...
    while True:
        ...
        for start_index in range(0, chunks_selected_this_cycle, batch_size):
            ...
            pending_batches = [(batch_df, max_embedding_input_chars)]

            while pending_batches:
                current_batch_df, current_max_input_chars = pending_batches.pop(0)
                response = None
                rate_limit_retries = 0

                while True:
                    input_texts = build_embedding_input_texts(
                        batch_df=current_batch_df,
                        max_input_chars=current_max_input_chars,
                    )
                    ...
                    try:
                        ...
                        yield {
                            ...
                            "chunks_per_openai_request": int(len(current_batch_df)),
                            "configured_chunks_per_openai_request": batch_size,
                            "max_batches_per_cycle": max_batches_per_run,
                            "max_chunks_per_cycle": limit_chunks_per_cycle,
                            "max_embedding_input_chars_per_chunk": current_max_input_chars,
                            ...
                            "adaptive_request_splits": int(adaptive_request_splits),
                            "adaptive_input_truncations": int(adaptive_input_truncations),
                        ...
                        if len(current_batch_df) > 1:
                            split_index = max(1, int(len(current_batch_df) / 2))
                            first_batch_df = current_batch_df.iloc[:split_index].copy()
                            second_batch_df = current_batch_df.iloc[split_index:].copy()
                            pending_batches.insert(0, (second_batch_df, current_max_input_chars))
                            pending_batches.insert(0, (first_batch_df, current_max_input_chars))
                            adaptive_request_splits += 1

                            yield {
                                "message": "OpenAI rejected the request size. Splitting the batch and continuing.",
                                "status": "request_too_large_batch_split",
                                "mode": "rebuild" if rebuild_inventory == "Y" else "update",
                                "embedding_model": embedding_model,
                                "embedding_dimension": embedding_dimension,
                                "rejected_batch_size": int(len(current_batch_df)),
                                "new_batch_sizes": [int(len(first_batch_df)), int(len(second_batch_df))],
                                "configured_chunks_per_openai_request": batch_size,
                                "max_embedding_input_chars_per_chunk": current_max_input_chars,
                                "remaining_chunks_before_run": remaining_before,
                                "chunks_selected_this_run": int(chunks_selected_this_run),
                        ...
                        reduced_max_input_chars = max(
                            min_embedding_input_chars,
                            int(current_max_input_chars / 2),
                        )

                        if reduced_max_input_chars < current_max_input_chars:
                            current_max_input_chars = reduced_max_input_chars
                            adaptive_input_truncations += 1

                            yield {
                                "message": "OpenAI rejected a single-chunk request size. Truncating the chunk input and retrying.",
                                "status": "request_too_large_input_truncated",
                                "mode": "rebuild" if rebuild_inventory == "Y" else "update",
                                "embedding_model": embedding_model,
                                "embedding_dimension": embedding_dimension,
                                "chunks_per_openai_request": 1,
                                "configured_chunks_per_openai_request": batch_size,
                                "max_embedding_input_chars_per_chunk": current_max_input_chars,
                                "min_embedding_input_chars_per_chunk": min_embedding_input_chars,
                                "remaining_chunks_before_run": remaining_before,
                                "chunks_selected_this_run": int(chunks_selected_this_run),
                                "chunks_embedded_this_run": int(chunks_embedded),
                                "batches_processed_this_run": int(batches_processed),
                        ...
                        yield {
                            "message": "Embedding stopped because one chunk remains too large after minimum truncation.",
                            "status": "single_chunk_too_large_stopped",
                            "mode": "rebuild" if rebuild_inventory == "Y" else "update",
                            "embedding_model": embedding_model,
                            "embedding_dimension": embedding_dimension,
                            "chunk_id": str(current_batch_df.iloc[0]["chunk_id"]),
                            "chunks_per_openai_request": 1,
                            "configured_chunks_per_openai_request": batch_size,
                            "max_embedding_input_chars_per_chunk": current_max_input_chars,
                            "min_embedding_input_chars_per_chunk": min_embedding_input_chars,
                            ...
                            "adaptive_input_truncations": int(adaptive_input_truncations),
                            "remaining_chunks_without_embedding": remaining_after_bad_request,
                            "recommended_action": "Inspect and shorten the reported chunk_id, or lower EMBED_MIN_INPUT_CHARS_PER_CHUNK.",
                            "error_message": str(e)[:1000],
                        }
                        return
                ...
                for item in response.data:
                    row = current_batch_df.iloc[item.index]
